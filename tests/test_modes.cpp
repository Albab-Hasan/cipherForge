...
TEST(BlockModes, CTREncryptDecrypt)
{
  constexpr std::array<uint8_t, 32> key = {
      0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe,
      0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
  ...
  const std::vector<uint8_t> plaintext = {'C', 'T', 'R', ' ', 'm', 'o', 'd', 'e', ' ', 't', 'e', 's', 't'};

  cf::AES256 aes{std::span<const uint8_t, 32>(key)};

  auto ciphertext = cf::ctr_encrypt(aes, iv, plaintext);
  auto decrypted = cf::ctr_decrypt(aes, iv, ciphertext);

  EXPECT_EQ(plaintext, decrypted);
}

TEST(BlockModes, CTREncryptDecryptEmptyInput)
{
  constexpr std::array<uint8_t, 32> key = {
      0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe,
      0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
...
TEST(BlockModes, CTREncryptDecryptLongInput)
{
  constexpr std::array<uint8_t, 32> key = {
      0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe,
      0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
  ...
  std::vector<uint8_t> plaintext(1024, 'C'); // 1KB of data
...
TEST(BlockModes, CTREncryptDecryptDifferentIV)
{
  constexpr std::array<uint8_t, 32> key = {
      0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe,
      0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
  ...
  constexpr std::array<uint8_t, 16> iv1 = {
      0x00, 0x01, 0x02, 0x03,
      0x04, 0x05, 0x06, 0x07,
      0x08, 0x09, 0x0a, 0x0b,
      0x0c, 0x0d, 0x0e, 0x0f};

  constexpr std::array<uint8_t, 16> iv2 = {
      0xff, 0xee, 0xdd, 0xcc,
      0xbb, 0xaa, 0x99, 0x88,
      0x77, 0x66, 0x55, 0x44,
      0x33, 0x22, 0x11, 0x00};

  const std::vector<uint8_t> plaintext = {'D', 'i', 'f', 'f', 'e', 'r', 'e', 'n', 't', ' ', 'I', 'V'};

  cf::AES256 aes{std::span<const uint8_t, 32>(key)};

  auto ciphertext1 = cf::ctr_encrypt(aes, iv1, plaintext);
  auto ciphertext2 = cf::ctr_encrypt(aes, iv2, plaintext);

  // Ciphertexts should be different when using different IVs
  EXPECT_NE(ciphertext1, ciphertext2);

  auto decrypted1 = cf::ctr_decrypt(aes, iv1, ciphertext1);
  auto decrypted2 = cf::ctr_decrypt(aes, iv2, ciphertext2);
